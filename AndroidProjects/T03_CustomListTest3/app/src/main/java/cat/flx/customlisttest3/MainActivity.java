package cat.flx.customlisttest3;
	...
	@Override protected void onCreate(Bundle savedInstanceState) {
		...
		while (cursor.moveToNext()) {
			...
			while (phones.moveToNext()) {
				...
				char newLetter = contact.name.charAt(0);
				if ((newLetter >= '0') && (newLetter <= '9')) newLetter = '#';
				if (newLetter != lastLetter) {
					Contact group = new Contact();
					group.isGroup = true;
					group.name = String.valueOf(newLetter);
					contacts.add(group);
	...
	private class ContactAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (!mustInflate) {
				...
				if (viewInfo.contact.isGroup != contact.isGroup) mustInflate = true;
			}
			if (mustInflate) {
				LayoutInflater inflater = (LayoutInflater) context.getSystemService(
						Context.LAYOUT_INFLATER_SERVICE);
				if (contact.isGroup)  
					view = inflater.inflate(R.layout.contact_list_group, parent, false);
				else 
